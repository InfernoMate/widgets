...
        sharedChatDiv.prepend(iconSpan);

        // Avatar laden und anzeigen (sharedChatAvatar MUSS oben per querySelector definiert sein!)
...
        }
    } 
    // Wenn die Einstellung 0 ist (also "Do not show"), brechen wir hier ab
    else if (showTwitchSharedChat === 0) {
        return;
    }
	}
...
async function TwitchCheer(data) {
	...
	titleDiv.innerText = `${username} hat ${bits} Bits gecheert`;
...
async function TwitchFollow(data) {
	...
	// Set the text
	const name = data.targetUser?.name || data.user_name || "Jemand";
	const login = data.targetUser?.login || data.user_login || "";
	...
	titleDiv.innerText = `${username} hat auf Folgen gedrückt`;
...
async function TwitchSub(data) {
	...
	if (!isPrime)
		titleDiv.innerText = `${username} hat auf Stufe ${subTier.charAt(0)} abonniert`;
	else
		titleDiv.innerText = `${username} nutzte den Prime-Sub`;
...
async function TwitchResub(data) {
	...
	if (!isPrime)
		titleDiv.innerText = `${username} hat auf Stufe ${subTier.charAt(0)} erneut abonniert (${cumulativeMonths} Monate)`;
	else
		titleDiv.innerText = `${username} nutzte den Prime-Sub (${cumulativeMonths} Monate)`;
...
async function TwitchGiftSub(data) {
	...
	titleDiv.innerText = `${username} hat ${recipient} ein Abo der Stufe ${subTier.charAt(0)} geschenkt`;
	if (cumlativeTotal > 0)
		contentDiv.innerText = `Insgesamt hat ${username} schon ${cumlativeTotal} Abos verschenkt!`;
...
async function TwitchRewardRedemption(data) {
	...
	titleDiv.innerHTML = `${username} hat ${rewardName} eingelöst ${channelPointIcon} ${cost}`;
...
async function TwitchCustomPowerUpRedemption(data) {
	...
	// Set the card background colors
	cardDiv.classList.add('twitch'); //setzt die normale Twitch Card Farbe !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ToDo das auswählbar machen und selbes für Kanalpunktebelohnungen

	// const baseColor = data.customPowerUp.backgroundColor; //setzt die Power Up Farbe, die bei Twitch eingestellt wurde als Card Farbe e.g., #3498db !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	titleDiv.innerHTML = `${username} hat ${powerUpName} eingelöst ${bitIcon} ${cost}`;
	contentDiv.innerText = `${userInput}`;
...
async function TwitchRaid(data) {
	...
	titleDiv.innerText = `${username} raidet`;
	contentDiv.innerText = `mit ${viewers} Zuschauern`;
...
async function TwitchWatchStreak(data) {
	...
	titleDiv.innerText = `${displayName} hat aktuell eine Watch-Streak von ${watchStreak} Streams! `;
...
function YouTubeNewSubscriber(data) {
	...
	// Set message text
    titleDiv.innerText = `⭐ ${data.user.name} hat abonniert!`;
...
function YouTubeSuperChat(data) {
	...
	// Set message text
	titleDiv.innerText = `🪙 ${data.user.name} hat einen Super Chat gesendet (${data.amount})`;
	if (data.message)
...
function YouTubeSuperSticker(data) {
	...
	titleDiv.innerHTML = `${user} hat einen Super Sticker gesendet (${amount})`;
...
function YouTubeNewSponsor(data) {
	...
	// Set message text
	titleDiv.innerText = `⭐ Neues Mitglied: ${data.levelName}`;
	contentDiv.innerText = `Willkommen ${data.user.name}!`;
...
function YouTubeGiftMembershipReceived(data) {
	...
	// Set message text
	titleDiv.innerText = `🎁 ${data.gifter.name} hat eine Mitgliedschaft verschenkt`;
	contentDiv.innerText = `an ${data.user.name} (${data.tier})!`;
...
async function StreamlabsDonation(data) {
	...
	titleDiv.innerText = `🪙 ${donater} hat ${currency}${formattedAmount} gespendet`;
	contentDiv.innerText = `${message}`;
...
function PatreonPledgeCreated(data) {
	...
	titleDiv.innerHTML = `${patreonIcon} ${user} ist Patreon beigetreten ($${amount})`;
...
function KofiDonation(data) {
	...
	if (currency == "EUR")
        titleDiv.innerHTML = `${kofiIcon} ${user} hat ${amount}€ gespendet`;
    else if (currency == "USD")
        titleDiv.innerHTML = `${kofiIcon} ${user} hat $${amount} gespendet`;
    else
        titleDiv.innerHTML = `${kofiIcon} ${user} hat ${currency} ${amount} gespendet`;
...
function KofiSubscription(data) {
	...
	if (currency == "EUR")
		titleDiv.innerHTML = `${kofiIcon} ${user} hat abonniert (${amount}€)`;
	else if (currency == "USD")
        titleDiv.innerHTML = `${kofiIcon} ${user} hat abonniert ($${amount})`;
	else
		titleDiv.innerHTML = `${kofiIcon} ${user} hat abonniert (${currency} ${amount})`;
...
function KofiResubscription(data) {
	...
	titleDiv.innerHTML = `${kofiIcon} ${user} hat erneut abonniert (${tier})`;
	if (message != null)
		contentDiv.innerHTML = `${message}`;
...
function KofiShopOrder(data) {
	...
	titleDiv.innerHTML = `${kofiIcon} ${user} hat ${itemTotal} Artikel auf Ko-fi bestellt ${formattedAmount}`;
	if (message != null)
		contentDiv.innerHTML = `${message}`;
...
function TipeeeStreamDonation(data) {
	...
	if (currency == "EUR")
		titleDiv.innerHTML = `${tipeeeStreamIcon} ${user} hat ${amount}€ gespendet`;
	else if (currency == "USD")
		titleDiv.innerHTML = `${tipeeeStreamIcon} ${user} hat $${amount} gespendet`;
	else
		titleDiv.innerHTML = `${tipeeeStreamIcon} ${user} hat ${currency} ${amount} gespendet`;
...
function FourthwallOrderPlaced(data) {
	...
	contents += `${user} hat ${item} bestellt`;
	if (itemsOrdered > 1)
		contents += ` und ${itemsOrdered - 1} weitere Artikel!`
...
function FourthwallGiftPurchase(data) {
	...
	contents += `Jemand hat verschenkt`;
...
function FourthwallGiftDrawStarted(data) {
	...
	// If the user ordered more than one item, write how many items they ordered
	contents += `🎁 ${itemName} Gewinnspiel!`;

	titleDiv.innerHTML = contents;
	contentDiv.innerHTML = `Schreibe 'join' in den nächsten ${durationSeconds} Sekunden in den Twitch Chat, um teilzunehmen!`;
	//contentDiv.style.display = `none`;
...
function FourthwallGiftDrawEnded(data) {
	...
	// If the user ordered more than one item, write how many items they ordered
	contents += `🥳 GEWINNSPIEL BEENDET 🥳`;
	//contents += `Congratulations ${GetWinnersList(data.gifts)}!`

	titleDiv.innerHTML = contents;
	contentDiv.innerHTML = `Herzlichen Glückwunsch ${GetWinnersList(data.gifts)}!`;
	//contentDiv.style.display = `none`;
...
async function KickSubscription(data) {
	...
	if (months <= 1)
		titleDiv.innerText = `${username} hat gerade zum ersten Mal abonniert!`;
	else
		titleDiv.innerText = `${username} hat erneut abonniert! (${months} Monate)`;
...
async function KickGiftedSubscriptions(data) {
	...
	titleDiv.innerText = `${gifter} hat ${giftedUsers.length} Abo${giftedUsers.length === 1 ? '' : 's'} an die Community verschenkt!`;
	contentDiv.innerText = `${gifts === 1 ? '' : `Insgesamt hat ${gifter} ${gifts} Abos im Kanal verschenkt.`}`;
...
async function KickGiftToUser(gifter, username) {
	...
	// Set the text
	titleDiv.innerText = `${gifter} hat ${username} ein Abo geschenkt`;
...
async function KickRewardRedeemed(data) {
	...
	titleDiv.innerHTML = `${username} hat ${rewardName} eingelöst`;
	contentDiv.innerText = `${userInput}`;
...
function TikTokFollow(data) {
	...
	titleDiv.innerHTML = `${tiktokIcon} ${user} hat das '+' weg gemacht`;
...
function TikTokSubscribe(data) {
	...
	titleDiv.innerHTML = `${tiktokIcon} ${user} hat für ${data.subMonth} Monate abonniert`;
...
function YouTubeThumbnailPreview(data) {
	...
	contentDiv.innerHTML = `von ${author}`;
