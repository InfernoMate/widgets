    ...
    websocket.onmessage = function (response) {
        try {
            ...
            switch (event) {
                ...
                //  break;
                case 'FollowEvent': // testen wegen pusher
...
async function TwitchCheer(data) {
    ...
    const message = `${username} hat ${bits} Bits gespendet`;
...
function TwitchFollow(data) { 
    ...
    const message = `${username} hat auf Folgen gedrückt`;
    ShowAlert(message, 'twitch');
}
...
async function TwitchSub(data) {
    ...
    if (!isPrime)
        message = `${username} hat mit Stufe ${subTier.charAt(0)} abonniert`;
    else
        message = `${username} nutzte den Prime-Sub`, 'twitch';
...
async function TwitchResub(data) {
    ...
    if (!isPrime)
        message = `${username} hat mit Stufe ${subTier.charAt(0)} erneut abonniert (${cumulativeMonths} Monate)`;
    else
        message = `${username} nutzte den Prime-Sub (${cumulativeMonths} Monate)`;
...
async function TwitchGiftSub(data) {
    ...
    let message = `🎁 ${username} hat ein Stufe ${subTier.charAt(0)} Abo an ${recipient} verschenkt`;
...
async function TwitchGiftBomb(data) {
    ...
    let message = `🎁 ${username} hat ${gifts} Stufe ${subTier} Abos verschenkt!`;
...
async function TwitchRewardRedemption(data) {
    ...
    let message = `${username} hat ${rewardName} eingelöst ${channelPointIcon} ${cost}`;
...
async function TwitchCustomPowerUpRedemption(data) {
	...
	let message = `${username} hat ${powerUpName} eingelöst ${bitIcon} ${cost}`;
...
async function TwitchRaid(data) {
    ...
    let message = `${username} raidet mit ${viewers} Zuschauern`;
...
async function TwitchWatchStreak(data) {
    ...
    let message = `${displayName} hat aktuell eine Serie von ${watchStreak} Streams!`;
...
function YouTubeNewSubscriber(data) {
    ...
    let message = `⭐ ${data.user.name} hat abonniert!`;
...
function YouTubeSuperChat(data) {
    ...
    let message = `🪙 ${data.user.name} hat einen Super Chat gesendet (${data.amount})`;
...
function YouTubeSuperSticker(data) {
    ...
    let message = `${data.user.name} hat einen Super Sticker gesendet (${data.amount})`;
...
function YouTubeNewSponsor(data) {
    ...
    // Set message text
    let message = `⭐ Neues Mitglied ${data.levelName} • Willkommen ${data.user.name}!`;
...
function YouTubeGiftMembershipReceived(data) {
    ...
    let message = `🎁 ${data.gifter.name} hat eine Mitgliedschaft an ${data.user.name} verschenkt (${data.tier})!`;
...
function StreamElementsTip(data) {
    ...
    let message = `🪙 ${donater} hat ${currency}${formattedAmount} gespendet`;
...
function PatreonPledgeCreated(data) {
    ...
    let message = `${patreonIcon} ${user} ist Patreon beigetreten ($${amount})`;
...
function KofiDonation(data) {
    ...
    let message = "";
    if (currency == "USD")
        message = `${kofiIcon} ${user} hat $${amount} gespendet`;
    else
        message = `${kofiIcon} ${user} hat ${amount} ${currency} gespendet`;
...
function KofiSubscription(data) {
    ...
    let message = "";
    if (currency == "USD")
        message = `${kofiIcon} ${user} hat abonniert ($${amount})`;
    else
        message = `${kofiIcon} ${user} hat abonniert (${currency} ${amount})`;
...
function KofiResubscription(data) {
    ...
    let message = `${kofiIcon} ${user} hat abonniert (${tier})`;
...
function KofiShopOrder(data) {
    ...
    message = `${kofiIcon} ${user} hat ${itemTotal} Artikel auf Ko-fi bestellt ${formattedAmount}`;
    ShowAlert(message, 'kofi');
}
...
function TipeeeStreamDonation(data) {
    ...
    let message = "";
    if (currency == "USD")
        message = `${tipeeeStreamIcon} ${user} hat $${amount} gespendet`;
    else
        message = `${tipeeeStreamIcon} ${user} hat ${amount} ${currency} gespendet`;
...
function FourthwallOrderPlaced(data) {
    ...
    message += `${user} hat ${item} bestellt`;
    if (itemsOrdered > 1)
        message += ` und ${itemsOrdered - 1} weitere(n) Artikel!`
...
function FourthwallDonation(data) {
    ...
    let message = "";
    if (currency == "USD")
        message = `${user} hat $${amount} gespendet`;
    else
        message = `${user} hat ${amount} ${currency} gespendet`;
...
function FourthwallSubscriptionPurchased(data) {
    ...
    let message = "";
    if (currency == "USD")
        message = `${user} hat für $${amount} abonniert`;
    else
        message = `${user} hat für ${currency} ${amount} abonniert`;
...
function FourthwallGiftDrawStarted(data) {
    ...
    // If the user ordered more than one item, write how many items they ordered
    message += `🎁 ${itemName} Giveaway! • Schreibe 'join' in den nächsten ${durationSeconds} Sekunden für eine Gewinnchance!`;
...
function FourthwallGiftDrawEnded(data) {
    ...
    let message = `🥳 GIVEAWAY BEENDET 🥳`;

    ShowAlert(message, 'fourthwall');

    message = `Herzlichen Glückwunsch ${GetWinnersList(data.gifts)}!`;
...
function KickSubscription(data) {
    ...
    let message = '';
    if (months <= 1)
        message = `${username} hat gerade zum ersten Mal abonniert!`;
    else
        message = `${username} hat erneut abonniert! (${months} Monate)`;
...
function KickGiftedSubscriptions(data) {
    ...
    if (giftedUsers.length <= 1)
        message = `${gifter} hat ein Abo an ${giftedUsers[0]} verschenkt`;
    else
        message = `${gifter} hat ${giftedUsers.length} Abo${giftedUsers.length === 1 ? '' : 's'} an die Community verschenkt!`;
...
function KickRewardRedeemed(data) {
    ...
    const message = `${username} hat ${rewardName} eingelöst`;
...
function KickStreamHost(data) {
    ...
    const message = `${username} raidet mit ${viewers} Zuschauern`;
...
function KickKicksGifted(data) {
    ...
    const message = ` ${giftImg} ${data.sender.username} hat ${data.gift.name} gesendet ${kicksImg} ${data.gift.amount}`;
...
function TikTokFollow(data) {   
    ...
    const message = `${data.nickname} hat auf Folgen gedrückt`;
...
function TikTokGift(data) {
    ...
    const message = `${data.nickname} hat ${giftImg}x${data.repeatCount} gesendet`;
...
function TikTokSubscribe(data) {
    ...
    const message = `${username} hat auf TikTok abonniert`;
...
function SetConnectionStatus(connected) {
    ...
    if (connected) {
        ...
        statusContainer.innerText = "Verbindet...";
