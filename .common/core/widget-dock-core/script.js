...
const sbActionOpenUrl = '15fe5dd9-a86c-4dae-96c3-ffa5cab34c0e';
...
async function OpenURLFromHeader(url) {
    await sbClient.doAction(
        action = {
            id: sbActionOpenUrl
        },
        args = {
...
function SetConnectionState(isConnected) {
    if (isConnected) {
        ...
        sbStatusButton.title = `Verbunden mit ${sbClient.info.name} (${sbClient.info.version})`;
        ...
        SetErrorMessage('Keine Verbindung zum Streamer.bot');
    }
}
...
if (configJson) {
    ...
    fetch(configJson)
        ...
        .then(config => {
            ...
            // Set the page title
            parent.document.title = `${config.title.main} ${config.title.subtitle}`;

            // Set the title label
            title.textContent = config.title.main;
            subtitle.textContent = config.title.subtitle;

            // Set the acknowledgement label
            acknowledgement.textContent = config.metadata.acknowledgement;
        })
            ...
            .then(async config => {
                ...
                if (config.requiredSbActions) {
                    ...
                    config.requiredSbActions.forEach(req => {
                        ...
                        statusLabel.textContent = exists ? 'Vorhanden' : 'Fehlt';
...
function SetRequiredActionState(isSuccess) {
    if (isSuccess) {
        ...
        sbActionsButton.title = `Alle Actions vorhanden`;
        ...
        sbActionsButton.title = `Dir fehlen Streamer.bot Actions`;
